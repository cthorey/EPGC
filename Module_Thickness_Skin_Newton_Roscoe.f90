...
!!!!!!!!  SUBROUTINE THICKNESS_NEWTON_SOLVER
  ...
  SUBROUTINE  THICKNESS_SKIN_NEWTON_ROSCOE(H,P,T,BL,Ts,Dt,Dr,M,dist,ray,el,grav,sigma,nu,delta0,&
    ...
    col=1
    CALL THICKNESS(ftmps,col,N,M,H,P,T,BL,Ts,Coeff,Dt,Dr,dist,ray,qa,el,grav,nu,delta0,Phi)
    col=2
    CALL THICKNESS(fguess,col,N,M,H,P,T,BL,Ts,Coeff,Dt,Dr,dist,ray,qa,el,grav,nu,delta0,Phi)
    ...
    CALL JACOBI_THICKNESS(a1,b1,c1,d1,e1,f1,g1,k1,l1,N,M,H,P,T,BL,Ts,Coeff,Dt,Dr,dist,ray,el,grav,nu,delta0,Phi)
  ...
  !-------------------------------------------------------------------------------------
  !-------------------------------------------------------------------------------------
  SUBROUTINE   THICKNESS(f,col,N,M,H,P,T,BL,Ts,Coeff,Dt,Dr,dist,ray,qa,el,grav,nu,delta0,Phi)
    ...
    DOUBLE PRECISION ,INTENT(IN) :: el,grav,nu,delta0,Phi
    ...
    DOUBLE PRECISION :: Ts_a2,Ts_a3,T_a2,T_a3
    DOUBLE PRECISION :: Ts_b2,Ts_b3,T_b2,T_b3
    ...
    DO i=1,N,1
       IF (i .NE. N) THEN
          ...
          phi_a = -48.0d0/35.0d0*Phi2*T_a2*delta_a3 + (16.0d0/5.0d0)*&
          & Phi2*T_a2*delta_a2*h_a - 14.0d0/5.0d0*Phi2*T_a2*delta_a&
          & *h_a2 + Phi2*T_a2*h_a3 + (8.0d0/7.0d0)*Phi2*T_a*Ts_a*&
          & delta_a3 - 12.0d0/5.0d0*Phi2*T_a*Ts_a*delta_a2*h_a + (8.0d0&
          & /5.0d0)*Phi2*T_a*Ts_a*delta_a*h_a2 + (8.0d0/5.0d0)*Phi2*T_a&
          & *delta_a3 - 4*Phi2*T_a*delta_a2*h_a + 4*Phi2*T_a*delta_a*&
          & h_a2 - 2*Phi2*T_a*h_a3 + (8.0d0/35.0d0)*Phi2*Ts_a2*&
          & delta_a3 - 4.0d0/5.0d0*Phi2*Ts_a2*delta_a2*h_a + (6.0d0/&
          & 5.0d0)*Phi2*Ts_a2*delta_a*h_a2 - 8.0d0/5.0d0*Phi2*Ts_a*&
          & delta_a3 + 4*Phi2*Ts_a*delta_a2*h_a - 4*Phi2*Ts_a*delta_a&
          & *h_a2 + Phi2*h_a3 - 8.0d0/5.0d0*Phi*T_a*delta_a3 + 4*Phi*&
          & T_a*delta_a2*h_a - 4*Phi*T_a*delta_a*h_a2 + 2*Phi*T_a*h_a3&
          & + (8.0d0/5.0d0)*Phi*Ts_a*delta_a3 - 4*Phi*Ts_a*delta_a2*h_a +&
          & 4*Phi*Ts_a*delta_a*h_a2 - 2*Phi*h_a3 + h_a3
       ...
       IF (i .NE. 1) THEN
          ...
          phi_b = -48.0d0/35.0d0*Phi2*T_b2*delta_b3 + (16.0d0/5.0d0)*&
          & Phi2*T_b2*delta_b2*h_b - 14.0d0/5.0d0*Phi2*T_b2*delta_b&
          & *h_b2 + Phi2*T_b2*h_b3 + (8.0d0/7.0d0)*Phi2*T_b*Ts_b*&
          & delta_b3 - 12.0d0/5.0d0*Phi2*T_b*Ts_b*delta_b2*h_b + (8.0d0&
          & /5.0d0)*Phi2*T_b*Ts_b*delta_b*h_b2 + (8.0d0/5.0d0)*Phi2*T_b&
          & *delta_b3 - 4*Phi2*T_b*delta_b2*h_b + 4*Phi2*T_b*delta_b*&
          & h_b2 - 2*Phi2*T_b*h_b3 + (8.0d0/35.0d0)*Phi2*Ts_b2*&
          & delta_b3 - 4.0d0/5.0d0*Phi2*Ts_b2*delta_b2*h_b + (6.0d0/&
          & 5.0d0)*Phi2*Ts_b2*delta_b*h_b2 - 8.0d0/5.0d0*Phi2*Ts_b*&
          & delta_b3 + 4*Phi2*Ts_b*delta_b2*h_b - 4*Phi2*Ts_b*delta_b&
          & *h_b2 + Phi2*h_b3 - 8.0d0/5.0d0*Phi*T_b*delta_b3 + 4*Phi*&
          & T_b*delta_b2*h_b - 4*Phi*T_b*delta_b*h_b2 + 2*Phi*T_b*h_b3&
          & + (8.0d0/5.0d0)*Phi*Ts_b*delta_b3 - 4*Phi*Ts_b*delta_b2*h_b +&
          & 4*Phi*Ts_b*delta_b*h_b2 - 2*Phi*h_b3 + h_b3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE JACOBI_THICKNESS(a,b,c,d,e,f,g,k,l,N,M,H,P,T,BL,Ts,Coeff,Dt,Dr,dist,ray,el,grav,nu,delta0,Phi)
    ...
    DOUBLE PRECISION :: Ts_b2,Ts_a2,T_a2,T_b2,Ts_b3,T_b3,Ts_a3,T_a3
    DOUBLE PRECISION :: Phi2
    ...
    DO i=1,N,1
       ...
       IF1: IF (i .NE. N) THEN
          ...
          dphia_dhi1 = (16.0d0/5.0d0)*Phi2*T_a2*delta_a2 - 28.0d0/&
          & 5.0d0*Phi2*T_a2*delta_a*hi - 28.0d0/5.0d0*Phi2*T_a2*&
          & delta_a*hia + 3*Phi2*T_a2*hi2 + 6*Phi2*T_a2*hi*hia + 3*&
          & Phi2*T_a2*hia2 - 12.0d0/5.0d0*Phi2*T_a*Ts_a*delta_a2 +&
          & (16.0d0/5.0d0)*Phi2*T_a*Ts_a*delta_a*hi + (16.0d0/5.0d0)*Phi2&
          & *T_a*Ts_a*delta_a*hia - 4*Phi2*T_a*delta_a2 + 8*Phi2*T_a*&
          & delta_a*hi + 8*Phi2*T_a*delta_a*hia - 6*Phi2*T_a*hi2 - 12*&
          & Phi2*T_a*hi*hia - 6*Phi2*T_a*hia2 - 4.0d0/5.0d0*Phi2*Ts_a2&
          & *delta_a2 + (12.0d0/5.0d0)*Phi2*Ts_a2*delta_a*hi + (&
          & 12.0d0/5.0d0)*Phi2*Ts_a2*delta_a*hia + 4*Phi2*Ts_a*delta_a2&
          &  - 8*Phi2*Ts_a*delta_a*hi - 8*Phi2*Ts_a*delta_a*hia + 3*&
          & Phi2*hi2 + 6*Phi2*hi*hia + 3*Phi2*hia2 + 4*Phi*T_a*&
          & delta_a2 - 8*Phi*T_a*delta_a*hi - 8*Phi*T_a*delta_a*hia + 6*Phi&
          & *T_a*hi2 + 12*Phi*T_a*hi*hia + 6*Phi*T_a*hia2 - 4*Phi*Ts_a*&
          & delta_a2 + 8*Phi*Ts_a*delta_a*hi + 8*Phi*Ts_a*delta_a*hia - 6*&
          & Phi*hi2 - 12*Phi*hi*hia - 6*Phi*hia2 + 3*hi2 + 6*hi*hia + 3&
          & *hia2

          dphia_dhi = (16.0d0/5.0d0)*Phi2*T_a2*delta_a2 - 28.0d0/5.0d0&
          & *Phi2*T_a2*delta_a*hi - 28.0d0/5.0d0*Phi2*T_a2*delta_a*&
          & hia + 3*Phi2*T_a2*hi2 + 6*Phi2*T_a2*hi*hia + 3*Phi2*&
          & T_a2*hia2 - 12.0d0/5.0d0*Phi2*T_a*Ts_a*delta_a2 + (16.0d0&
          & /5.0d0)*Phi2*T_a*Ts_a*delta_a*hi + (16.0d0/5.0d0)*Phi2*T_a*&
          & Ts_a*delta_a*hia - 4*Phi2*T_a*delta_a2 + 8*Phi2*T_a*delta_a&
          & *hi + 8*Phi2*T_a*delta_a*hia - 6*Phi2*T_a*hi2 - 12*Phi2*&
          & T_a*hi*hia - 6*Phi2*T_a*hia2 - 4.0d0/5.0d0*Phi2*Ts_a2*&
          & delta_a2 + (12.0d0/5.0d0)*Phi2*Ts_a2*delta_a*hi + (12.0d0/&
          & 5.0d0)*Phi2*Ts_a2*delta_a*hia + 4*Phi2*Ts_a*delta_a2 - 8*&
          & Phi2*Ts_a*delta_a*hi - 8*Phi2*Ts_a*delta_a*hia + 3*Phi2*hi2&
          &  + 6*Phi2*hi*hia + 3*Phi2*hia2 + 4*Phi*T_a*delta_a2 -&
          & 8*Phi*T_a*delta_a*hi - 8*Phi*T_a*delta_a*hia + 6*Phi*T_a*hi2 +&
          & 12*Phi*T_a*hi*hia + 6*Phi*T_a*hia2 - 4*Phi*Ts_a*delta_a2 + 8*&
          & Phi*Ts_a*delta_a*hi + 8*Phi*Ts_a*delta_a*hia - 6*Phi*hi2 - 12*&
          & Phi*hi*hia - 6*Phi*hia2 + 3*hi2 + 6*hi*hia + 3*hia2
       ...
       IF2: IF (i .NE. 1) THEN
          ...
          dphib_dhi = (16.0d0/5.0d0)*Phi2*T_b2*delta_b2 - 28.0d0/5.0d0&
               & *Phi2*T_b2*delta_b*hi - 28.0d0/5.0d0*Phi2*T_b2*delta_b*&
               & hib + 3*Phi2*T_b2*hi2 + 6*Phi2*T_b2*hi*hib + 3*Phi2*&
               & T_b2*hib2 - 12.0d0/5.0d0*Phi2*T_b*Ts_b*delta_b2 + (16.0d0&
               & /5.0d0)*Phi2*T_b*Ts_b*delta_b*hi + (16.0d0/5.0d0)*Phi2*T_b*&
               & Ts_b*delta_b*hib - 4*Phi2*T_b*delta_b2 + 8*Phi2*T_b*delta_b&
               & *hi + 8*Phi2*T_b*delta_b*hib - 6*Phi2*T_b*hi2 - 12*Phi2*&
               & T_b*hi*hib - 6*Phi2*T_b*hib2 - 4.0d0/5.0d0*Phi2*Ts_b2*&
               & delta_b2 + (12.0d0/5.0d0)*Phi2*Ts_b2*delta_b*hi + (12.0d0/&
               & 5.0d0)*Phi2*Ts_b2*delta_b*hib + 4*Phi2*Ts_b*delta_b2 - 8*&
               & Phi2*Ts_b*delta_b*hi - 8*Phi2*Ts_b*delta_b*hib + 3*Phi2*hi2&
               &  + 6*Phi2*hi*hib + 3*Phi2*hib2 + 4*Phi*T_b*delta_b2 -&
               & 8*Phi*T_b*delta_b*hi - 8*Phi*T_b*delta_b*hib + 6*Phi*T_b*hi2 +&
               & 12*Phi*T_b*hi*hib + 6*Phi*T_b*hib2 - 4*Phi*Ts_b*delta_b2 + 8*&
               & Phi*Ts_b*delta_b*hi + 8*Phi*Ts_b*delta_b*hib - 6*Phi*hi2 - 12*&
               & Phi*hi*hib - 6*Phi*hib2 + 3*hi2 + 6*hi*hib + 3*hib2
          
          dphib_dhi1 = (16.0d0/5.0d0)*Phi2*T_b2*delta_b2 - 28.0d0/&
               & 5.0d0*Phi2*T_b2*delta_b*hi - 28.0d0/5.0d0*Phi2*T_b2*&
               & delta_b*hib + 3*Phi2*T_b2*hi2 + 6*Phi2*T_b2*hi*hib + 3*&
               & Phi2*T_b2*hib2 - 12.0d0/5.0d0*Phi2*T_b*Ts_b*delta_b2 +&
               & (16.0d0/5.0d0)*Phi2*T_b*Ts_b*delta_b*hi + (16.0d0/5.0d0)*Phi2&
               & *T_b*Ts_b*delta_b*hib - 4*Phi2*T_b*delta_b2 + 8*Phi2*T_b*&
               & delta_b*hi + 8*Phi2*T_b*delta_b*hib - 6*Phi2*T_b*hi2 - 12*&
               & Phi2*T_b*hi*hib - 6*Phi2*T_b*hib2 - 4.0d0/5.0d0*Phi2*Ts_b2&
               & *delta_b2 + (12.0d0/5.0d0)*Phi2*Ts_b2*delta_b*hi + (&
               & 12.0d0/5.0d0)*Phi2*Ts_b2*delta_b*hib + 4*Phi2*Ts_b*delta_b2&
               &  - 8*Phi2*Ts_b*delta_b*hi - 8*Phi2*Ts_b*delta_b*hib + 3*&
               & Phi2*hi2 + 6*Phi2*hi*hib + 3*Phi2*hib2 + 4*Phi*T_b*&
               & delta_b2 - 8*Phi*T_b*delta_b*hi - 8*Phi*T_b*delta_b*hib + 6*Phi&
               & *T_b*hi2 + 12*Phi*T_b*hi*hib + 6*Phi*T_b*hib2 - 4*Phi*Ts_b*&
               & delta_b2 + 8*Phi*Ts_b*delta_b*hi + 8*Phi*Ts_b*delta_b*hib - 6*&
               & Phi*hi2 - 12*Phi*hi*hib - 6*Phi*hib2 + 3*hi2 + 6*hi*hib + 3&
               & *hib2
