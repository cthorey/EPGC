...
!!!!!!!!  Subroutine pour résoudre eqaution de la chaleur à l'aide d'une schemé différence fini !
  ...
  SUBROUTINE THERMAL_GFD_SOLVER(Xi,H,P,T,Ts,BL,Dt,Dr,theta,dist,ray,M,sigma,nu,Pe,psi,delta0,el,grav,N1,F_err,z,tmps)
    ...
    ALLOCATE(Xi_tmps(1:N),stat=err1)
    IF (err1>1) THEN
       PRINT*, 'Erreur alloc xi_tmps-xi_guess'; STOP
    END IF

    ! col=1
    ! CALL TEMPERATURE_CFD_BALMFORTH(Xi_tmps,col,N,Xi,H,T,Ts,BL,P,dist,ray,Dr,nu,Pe,delta0,el,grav,N1,tmps,Dt)

    !Systeme a inverser
    ALLOCATE(a(1:N),b(1:N),c(1:N),d(1:N),e(1:N),S(1:N),stat=err1)
    ALLOCATE(f(1:N),g(1:N),k(1:N),l(1:N),stat=err1)
    IF (err1>1) THEN
       PRINT*, 'Erreur allocation dans coeff Temperature'; STOP
    END IF
    CALL FILLING_MATRIX_2(a,b,c,d,e,f,g,k,l,S,Xi_tmps,N,H,BL,T,Ts,Xi,P,Dr,dist,ray,&
         &nu,N1,Pe,delta0,el,grav,tmps,Dt,theta,psi)
    ...
    CALL XI_SPLIT_2(Xi,T,BL,Ts,H,N,delta0,Dt,tmps,N1,Pe,el)
    Xi(N:,3) = H(N:,3)/2.D0
    T(N:,3) = 0.d0
    Ts(N:,3) =0.d0
    BL(N:,3) = H(N:,3)/2.d0
  ...
  SUBROUTINE XI_SPLIT_2(Xi,T,BL,Ts,H,N,delta0,Dt,tmps,N1,Pe,el)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE TEMPERATURE_CFD_BALMFORTH(f,col,N,Xi,H,T,Ts,BL,P,dist,ray,Dr,nu,Pe,delta0,el,grav,N1,tmps,Dt)
    ...
    DOUBLE PRECISION ,INTENT(IN) :: nu,Pe,delta0,el,grav,N1,tmps,Dt
  ...
  END SUBROUTINE TEMPERATURE_CFD_BALMFORTH
  ...
  SUBROUTINE FILLING_MATRIX(a,b,c,S,Xi_tmps,N,H,BL,T,Ts,Xi,P,Dr,dist,ray,nu,N1,Pe,delta0,el,grav,tmps,Dt,theta,psi)
  ...
  IMPLICIT NONE
    ...
    ! Tableaux
    DOUBLE PRECISION ,DIMENSION(:) , INTENT(INOUT) :: a,b,c,S,Xi_tmps
    DOUBLE PRECISION ,DIMENSION(:,:), INTENT(IN) :: H,Xi,T,Ts,BL,P
    DOUBLE PRECISION ,DIMENSION(:), INTENT(IN) :: dist,ray
    ...
    DO i=1,N,1   
       ...
       IF3:IF (i==1) THEN
          a(i) = 0.d0
          b(i) = (1.d0-Psi)-Dt*theta*Ai*Omega_a
          c(i) = 0.d0
          S(i) = (1.d0-theta)*Xi_tmps(i)+(1.d0-Psi)*Xi(i,1)+Dt*theta*(Ai*Sigma_a-Bi*Sigma_b+loss)
       ELSEIF (i == N) THEN
          a(i) =0.d0
          b(i) = (1.d0-Psi)
          S(i) = Dt*theta*loss
       ELSE
          a(i) = Dt*theta*Bi*Omega_b
          b(i) = (1.d0-Psi)-Dt*theta*Ai*Omega_a
          c(i) = 0.d0
          S(i) = (1.d0-theta)*Xi_tmps(i)+(1.d0-Psi)*Xi(i,1)+Dt*theta*(Ai*Sigma_a-Bi*Sigma_b+loss)
       END IF IF3
 ...
 SUBROUTINE FILLING_MATRIX_2(a,b,c,d,e,f,g,k,l,S,Xi_tmps,N,H,BL,T,Ts,Xi,P,Dr,dist,ray,&
      &nu,N1,Pe,delta0,el,grav,tmps,Dt,theta,psi)

    !*****************************************************************
    ! On essay avec un third upwaind order scheme
    !*****************************************************************
  IMPLICIT NONE

    ! Tableaux
    DOUBLE PRECISION ,DIMENSION(:) , INTENT(INOUT) :: a,b,c,d,e,f,g,k,l,S,Xi_tmps
    DOUBLE PRECISION ,DIMENSION(:,:), INTENT(IN) :: H,Xi,T,Ts,BL,P
    DOUBLE PRECISION ,DIMENSION(:), INTENT(IN) :: dist,ray
    ...
    DO i=1,N,1   
       ...
       ! beta = N1*Pe**(-0.5d0)/(sqrt(pi*(tmps+Dt)))
       ! loss = Pe*beta*Ts(i,col)
       loss = 2*Pe*T(i,col)/BL(i,col)*Dt
      
       IF3:IF (i==1) THEN
          a(i) =0.d0
          c(i) = 0.d0
          d(i) = 0.d0
          e(i) = (1.d0-Psi)+(2.d0*Bi-5.d0*Ai)/6.d0+(5.d0*Bi+Ai)/6.d0
          f(i) = -2*Ai/6.d0
          f(i) = 0.d0; g(i) =0.d0; k(i) =0.d0; l(i)=0.d0
          S(i) = (1.d0-Psi)*Xi(i,1)+Ai*Sigma_a-Bi*Sigma_b+loss
       ELSEIF (i==2) THEN
          a(i) =0.d0
          c(i) = 0.d0
          d(i) = (5.d0*Bi+Ai)/6.d0-Bi/6.d0
          e(i) = (1.d0-Psi)+(2.d0*Bi-5.d0*Ai)/6.d0
          f(i) = -2*Ai/6.d0
          f(i) = 0.d0; g(i) =0.d0; k(i) =0.d0; l(i)=0.d0
          S(i) = (1.d0-Psi)*Xi(i,1)+Ai*Sigma_a-Bi*Sigma_b+loss
       ELSEIF (i == N) THEN
